...
import com.venink.slec.dao.OrderMapper;
...
public class OrderServiceImpl extends BaseServiceImpl<Order, Integer> implements OrderService {
    ...
    @Override
    public BaseDao<Order, Integer> getDao() {
        return orderMapper;
    }
    ...
    public boolean modifyOrderById(Order order) {
        if (order.getOrderId() != null) {
            List<Order> list = orderMapper.queryOrderByOrderId(order.getOrderId());
            ...
            int effectNum = orderMapper.insertOrder(order2);
            if (effectNum > 0) {
                return true;
            } else{
